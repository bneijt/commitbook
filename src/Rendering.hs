...
combineImages :: ImageMatrix -> [IO Pixbuf] -> Surface -> IO ()
combineImages matrix images surface = do
    ...
    surfaceWriteToPNG surface "z.png"

renderImages :: Int -> Int -> [String] -> IO()
renderImages width height imageFileNames = do
    initGUI -- Initialize GDK for jpeg loading
    withImageSurface FormatRGB24 width height (combineImages (ImageMatrix (fromIntegral width) (fromIntegral height) 3 4) (map pixbufNewFromFile imageFileNames))
